...
| ~~B3~~ RESOLVED | Latest 2 majors of Chrome/Edge/Firefox/Safari; no IE, no legacy Edge | ~~Task 15~~ | Task 15 defines design tokens and the logical-property CSS strategy, whose fallbacks depend on the supported browser floor | Tasks 1–14 proceed on the Angular default browserslist. Task 15 cannot start |
| ~~B2~~ RESOLVED | Temporary NEUTRAL SEMANTIC tokens; no invented branding; components consume semantic tokens only | ~~Task 15~~ | Task 15 generates the PrimeNG theme *from* tokens; the token model is what every later component consumes | Task 15 cannot start unless the user explicitly approves temporary placeholder tokens |
